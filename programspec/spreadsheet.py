...
class Spreadsheet:
    ...
    def err(self, err, err_args=None):
        ...
        self._ok = False
        if type(err) == tuple:
            err = errors.error(err, err_args)
        else:
            err = errors.error(errors.generic_issue, {'message': err})
        if 'debug' in options:
            print('{} {}: {}'.format(*err), file=sys.stderr)

    @staticmethod
    def warn(warning, warn_args=None):
        global options
        if type(warning) == tuple:
            err = errors.error(warning, warn_args)
        else:
            err = errors.error(errors.generic_warning, {'message': warning})
        if 'debug' in options:
            print('{} {}: {}'.format(*err), file=sys.stderr)
            ...
            if name in columns_of_interest:
                ...
                if name in columns_found:
                    ...
                    if name not in column_errors:
                        self.err(errors.duplicate_columns, {'column': name, 'sheet': sheet_name})
    ...
    def _validate_basic_sheets(self):
        ...
        for sheet in required_sheets:
            if sheet not in wb_sheets:
                self.err(errors.missing_sheet, {'sheet': sheet})
            else:
    ...
    def _validate_components_sheet(self):
        ...
        for name in filtered_comps:
            if name not in wb_sheets:
                self.err(errors.missing_sheet, {'sheet': name})
            else:
    ...
    def _check_component_in_recipients(self):
        for component_name in self._components:
            recipients = self._recipients[component_name]
            for recipient in recipients:
                if recipient['component'] != component_name:
                    args = {'community': recipient['community'], 'group': recipient['group_name'],
                            'row': recipient['row_num'],
                            'incorrect': recipient['component'], 'component': component_name}
    ...
    def _validate(self):
        ...
        try:
            ...
            self.err(errors.validation_exception, {'message': exc_msg})
