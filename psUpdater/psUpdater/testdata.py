...
import db
from psUpdater import XlsExporter, Spec
from psUpdater.ImportProcessor import ImportProcessor
from psUpdater.Spec import Program
from psUpdater.SpecCompare import SpecCompare
...
def test_other():
    ...
    update = json.loads(testdata.test_json)

    program_spec: Spec.Program = Spec.progspec_from_json(program, update)

    importer: ImportProcessor = ImportProcessor(program, program_spec)
...
def test_get_put(programid: str):
    ...
    result2 = lambda_router(event, context)
    print(result2)
...
def run_tests(programid: str):
    ...
    spec: Program = Spec.progspec_from_json(programid, content)

    differ: SpecCompare = SpecCompare(exporter.program_spec, spec)
    diffs = differ.diff()
    print(diffs)
