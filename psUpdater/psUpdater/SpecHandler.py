...
def download_handler(programid: QueryStringParam, artifact: QueryStringParam, aslink: QueryStringParam, email: Claim):
    ...
    if artifact == UNPUBLISHED_ARTIFACT_NAME:
        ...
        exporter: XlsExporter.Exporter = XlsExporter.Exporter(program_id=programid, engine=engine)
        exporter.read_from_database()
        ok, _ = exporter.save_unpublished(bucket=PROGSPEC_BUCKET, metadata=metadata)
    if key:
...
PROGSPEC_HANDLERS = {'upload': upload_handler,          # inserts body to S3 'pending'
                     'compare': compare_handler,
                     'accept': accept_handler,          # loads s3 'pending', inserts to SQL
                     'publish': publish_handler,        # loads SQL, writes artifacts to S3
                     'download': download_handler,      # returns link to spec in S3
                     'list': list_objects,
                     'validate': validation_handler,
                     'echo': echo_handler,
                     'get_content': get_content,        # loads SQL, returns json
                     'put_content': put_content,        # gets spec from body, inserts to SQL
                     }
